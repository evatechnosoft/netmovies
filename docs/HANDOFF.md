...
> cache'te 90 sn sınırını aşıyor. İkinci koşu 450 içerik verir — kusur değil.

## 🔬 EMÜLATÖR — cihaz beklemeden test

`eva_test` AVD (API 35) ev ağına erişiyor, gerçek katalogla açılıyor. **Yatay
çevir**, yoksa TV yerleşimi değerlendirilemez:
...
"$ADB" install -r -t <apk>
"$ADB" shell cmd package compile -m verify -f com.evaitec.netmovies.tv   # VerifyError kapisi
...
"$ADB" shell input keyevent 22   ·   "$ADB" shell input keyevent --longpress 23
```

**Emülatörün sınırı:** oynatıcı açılıyor, `resolve_sources` dönüyor, ama görüntü
...
`PadBtn`leri (index). Kontroller açıkken AŞAĞI ok `false` dönüyor, odağın
`IconBtn`lere inmesi bekleniyordu — inmiyor: kök kutu `.focusable()` olduğu için
odak araması oraya gitmiyor. AŞAĞI ölü kalıyordu (Dean: "sarma ileri geri sonraki
bölüm dolaşmıyor, odak yok butonlarda"). Düzeltme: o dal silindi (AŞAĞI →
`OPEN_BAR` → QuickPad), `IconBtn`den `.focusable()`/`.clickable()` kaldırıldı —
odak alamayan düğme artık odak alacakmış gibi durmuyor. **Gezilebilir tek desen
QuickPad'in index'i.**

**Poster uzun-bas kartı** (Dean'in tarifi): dikey menü yerine üç sütun —
SOL bölümler · ORTA poster + yıl/puan/tür + özet + dil rozetleri · SAĞ listeler ·
ALT Oynat. Yön tuşları: ◀ bölümler, ▶ listeler, ▼ (sütun sonunda) oynat, ▲ özet,
OK seç, GERİ kapat. Seçim yine index'le — Compose odağı kullanılmıyor.
...
- **Çökme şeridine odaklanabilir düğme koymak** — tuşu `dispatchKeyEvent`te yakala.
- **Media3'ün hazır UI'si** (0.9.0'da denendi, geri alındı): overlay'i götürür.
- PlayerView'e odak vermek — `isFocusable=false` + `FOCUS_BLOCK_DESCENDANTS` kalsın.
- `resolve_sources`'a bölüm İNDEKSİ göndermek — adres gönder.
- AndroidView `factory` içinden Compose state yazmak.
- `assembleRelease` → imzasız APK; **`assembleDebug`** kullan.
- `sha256sum "$yol"` → ters bölü ekler; `sha256sum < dosya` kullan.
...
- Git Bash heredoc'una uzun Kotlin/Markdown gömmek → parse hatası; Write ile yaz.
- Python (Windows) `/tmp` görmez; scratchpad'in tam yolunu ver.

## Cevaplanmamış / açık

- Oynatıcı emülatörde görüntü gelmeden kapanıyor (toast). TV'de oynuyor →
  emülatör kusuru olabilir; TV'de de olursa: `docker logs netmovies-engine | grep resolve:`
  ve `curl -s localhost:3310/api/v1/client_log`.
- Kartta ▲ (ORTA/özet) yalnız özeti uzatıyor — Dean başka bir şey istemiş olabilir.
- "değişebilir şekilde ekle" (Dean) — kart tuşları buton eşlemeye bağlanmadı,
  sabit. İstenirse `input/RemoteInput.kt` üzerinden eşlenebilir.
...
- SezonlukDizi ekli ve çalışıyor: domain `sezonlukdizi.cc` (`.org` değil).
...
bölümler · orta özet/dil · sağ listeler · alt oynat), emülatörde uçtan uca denendi.
0.9.7 üç dağıtım yerinde yayında. OYNATICI İÇİ DAVRANIŞ CİHAZDA DENENMEDİ.

Önce docs/HANDOFF.md oku ve "Doğrula" bloğunu koş; repo ile doküman çelişirse repo
doğrudur. Emülatörü YATAY çevirmeden TV yerleşimine bakma.
...
Yayın kuralı: kanıt yeşilse SORMADAN yayınla (yerel OTA + GitHub release --target
