...
import { posterUrl } from '../utils/poster.min.js';
...
class ContentBrowser {
    ...
    _createPosterCard(item) {
        ...
        // Kaynak → proxy cache → TMDB zinciri sunucuda çalışır; burada tek URL
        // yeter, onerror yalnızca son çare placeholder'ı gösterir.
        const src = posterUrl(item.poster, item.title);
        if (src) {
            const img = createElement('img', { src, alt: item.title || '', loading: 'lazy' });
