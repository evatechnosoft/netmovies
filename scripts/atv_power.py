...
Neden ayrı bir süreç: stream konteyneri Docker Desktop'ın NAT'ı arkasında,
ev ağındaki cihazlara TCP açamıyor (`192.168.1.x` her portta timeout). Kutunun
kumanda protokolü (Android TV Remote v2, 6466/6467 — Google TV / Xiaomi Home
uygulamasının kullandığı protokol) ancak aynı ağdaki bir süreçten konuşulabilir,
o yüzden köprü HOST'ta çalışır ve stream ona `host.docker.internal` üzerinden
uğrar.
...
Uçlar:  GET /saglik · GET /guc (aç-kapa) · GET /tus/<AD>  (ör. HOME, BACK, DPAD_UP)
"""
...
class Kopru:
    ...
    def durum(self) -> dict:
        return {"host": KUTU_IP, "acik": self.tv.is_on, "cihaz": self.tv.device_info}

    def tus(self, ad: str) -> None:
        # Kapalıyken POWER'ı TCP ile göndermek işe yaramaz (kutu dinlemiyor):
        # kütüphane uyandırmayı kendi hallediyorsa is_on zaten True döner.
        self.tv.send_key_command(ad)
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            try:
                if yol in ("/saglik", ""):
                    ...
                    kopru.tus("POWER")
                    self._yanit(200, {"ok": True, "gonderildi": "POWER"})
